...
export default function Card(props){
    ...
    const [rightbtncol, setRightbtncol] = useState("click_col")
        ...
        <div className = "maincard flex my-2 h-auto items-center reg_shadow white_col rounded-lg flex-col md:my-3 md:p-3 lg:my-4 xl:flex-row">
            ...
            <div className="w-full mx-auto mt-2 xl:px-auto xl:my-2 xl:w-[40%]">
                ...
                <div className="mx-auto flex justify-center items-center">  
                    {/* MAIN IMAGES DIV */}
                    <div className="overflow-hidden relative rounded-md w-[7rem] h-[7rem] md:w-[9rem] md:h-[9rem] xl:w-[10rem] xl:h-[10rem] xl:rounded-lg">
                        {/* SLIDING IMAGES DIV */}
                        <div className={"absolute w-[inherit] h-[inherit] flex flex-row imgtime " + slide}>
                            {
                                
                                props.src.map((item)=>{
                                    return (
                                        <img key = {uuidv4()} className = "flex-shrink-0 w-[inherit] h-[inherit] " src = {item.img} alt = {item.alt}/>
                                    )
                                })
                            }
                        </div>
                ...
                <div className="h-auto my-2 mx-1 flex flex-row justify-center items-center">
                    {/* SLIDE LEFT BUTTON */}
                    <button className = {"m-2 hover:opacity-80 rounded-lg p-[8px] md:p-[10px] xl:p-[12px] xl:rounded-xl " + leftbtncol}
                    onClick={()=>{
                        if(pos > 0){
                            setPos(pos - 1)
                            leftslide();
                    ...
                    <div className = "w-max flex flex-row justify-between">
                        {
                            props.src.map((item)=>{
                                if(pos === item.id){
                                    return (
                                        <span key = {uuidv4()} className= {"mx-1 w-1 h-1 xl:mx-2 xl:w-2 xl:h-2 rounded-full theme_col" } ></span>
                                    )
                                }
                                else{
                                    return (
                                        <span key = {uuidv4()} className= {"mx-1 w-1 h-1 xl:mx-2 xl:w-2 xl:h-2 rounded-full click_col" } ></span>
                                    )
                                }
                            })
                        }
                    ...
                    {/* SLIDE RIGHT BUTTON */}
                    <button className = {"m-2 hover:opacity-80 rounded-lg p-[8px] md:p-[10px] xl:p-[12px] xl:rounded-xl " + rightbtncol} 
                    onClick={()=>{
            ...
            <div className="w-full h-full p-[8px] xl:p-[16px] flex flex-row flex-wrap xl:w-[60%]">
                ...
                <div className="w-full self-start">
                    {/* TITLE */}
                    <h2 className = "md:my-[3px] xl:my-[6px] text">
                        {props.title}
                    </h2>
                    {/* SUBTITLE */}
                ...
                <div className="w-full self-end">
                    ...
                    <button className="md:my-[3px] xl:my-[6px] flex flex-row items-center"
                        onClick={()=>{props.OpenSpec(props.title, props.subtitle, props.keyfeat, props.spec)}}>
                        <h4 className="others specs text-left">
                            View specifications
                        </h4>
                        <img className = "mx-1 specs" src = {bluearrow} alt = "left arrow" />
                    </button>
                    {/* PRICE */}
                    <h2 className="my-[3px] xl:my-[6px] price">
                        {"₦" + props.price}
                    </h2>
                    {/* STARS AND BUTTON */}
                    <div className="my-[2px] md:my-[3px] xl:my-[6px] flex flex-col items-start md:flex-row md:items-center justify-between">
                        <img className = "my-[3px] xl:my-[6px] md:w-[76.88px] md:h-[15px] xl:w-[116px] xl:h-[22.62px]" src = {props.star} alt = "rating stars"/>

                        {/* PURCHASE BUTTON */}
                        <a href={props.prodlink} type = "button" onClick={()=>{console.log(props.prodlink); setAnimate("animate")}} onTouchEnd = {()=>{setAnimate("")}} className={"mt-[6px] w-full md:w-auto pur_btn theme_col white " + animate}>
                            <h2 className="w-full text-center text_semibold">
                                Purchase
                            </h2>
                        </a>
                    </div>
